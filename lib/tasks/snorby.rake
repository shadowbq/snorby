...
namespace :snorby do
  ...
  desc 'Job - Sensor Cache'
  task :job_sensor_cache => :environment do
    if Snorby::Worker.running?
      puts "* Adding SensorCachejob to the queue"
      Snorby::Jobs::SensorCacheJob.new(true).perform
    else
      STDERR.puts "[x] Error: unable to find delayed_worker process to attach job."
      exit -1
    end
  end

  desc 'Job - GeoipUpdatedb'
  task :job_geoipUpdate => :environment do
    if Snorby::Worker.running?
      puts "* Adding GeoipUpdatedbjob to the queue"
